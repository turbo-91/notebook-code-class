...
//     headline.textContent = movie.title.toUpperCase();
//     description.textContent = movie.description;
//     image.setAttribute("src", movie.url);
//     image.addEventListener("click", () => {
//         console.log(idx);
//     });
//     movieCard.append(headline, image, description);
//     document.body.append(movieCard);
// });
...
// doubleNumbers using a map
const doubledNumbersMap = numbers.map((number) => {
    return number * 2;
});
console.log("doubledNumbersMap", doubledNumbersMap);
...
const bestRankedMovieTitles = movies
    .filter((movie) => movie.imdbRating > 8)
    .map((movie) => movie.title);

console.log("bestRankedMovieTitles", bestRankedMovieTitles);
